...
export class AppComponent {
  ...
  setpixels() {
    ...
    this.areaInfo.forEach((x) => {
      ...
      areaCooradinate.rectangleId = x.rectangleId;
  ...
  pageRendered(event) {
    ...
    // const rectElem = this.areaInfo.find(
    //   (f) => f.pageNumber === this.indexOfPage
    // );

    this.areaInfoInPixels.forEach((x) => {
      if (x.pageNumber == this.indexOfPage) {
        if (typeof x !== undefined) {
          ...
          rect.style.left = x.rect.x1 + 'px';
          rect.style.top = x.rect.y1 + 'px';
          rect.style.width = x.rect.width + 'px';
          rect.style.height = x.rect.height + 'px';
